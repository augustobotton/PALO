...
# Suponha que atm_padrao esteja definida e a classe ModeloAtmosferico também
def atm_padrao(h, v, lc, dT):
	...
	for j in range(1, i + 1):
		if a[j - 1] != 0:  # Verifica se a camada nao eh isotermica
			...
			Pi = px * np.exp(
				-(g0 / (Ri[j] * Ti[j - 1])) * (hi[j] - hi[j - 1]) * (1 - (beta / 2) * (hi[j] - hi[j - 1])))
			px = Pi  # O valor atual sera o valor anterior na proxima iteracao
...
# Instancia a classe com os mesmos dados usados na função original
modelo = ModeloAtmosferico(r'C:\Users\gt_po\Documents\tcc\mecvooespacial\src\domain\modelos\planeta\dados_JSON_planetas\dados_atmosfericos_terra.json')

# Define um conjunto de testes
altitudes = np.linspace(0, 2000e3, num=50)  # Altitudes de 0 a 2000 km
velocidades = [100, 500, 1000, 2000]  # Diferentes velocidades
lc = 10  # Comprimento característico do veículo
dT = 0  # Variação de temperatura local

for h in altitudes:
    for v in velocidades:
        # Calcula os resultados com a função original
        resultado_funcao = atm_padrao(h, v, lc, dT)

        # Calcula os resultados com a classe
        resultado_classe = modelo.calcula(h, v, lc, dT)

        # Compara os resultados (aqui estou usando apenas a temperatura cinética como exemplo)
        if not np.isclose(resultado_funcao[8], resultado_classe[8], rtol=1e-5):
            print(f"Discrepância encontrada na altitude {h} m e velocidade {v} m/s:")
            print(f"Função original: {resultado_funcao[8]}")
            print(f"Classe: {resultado_classe[8]}")
