...
class Simulacao:
    ...
    def _calcular_condicoes_azimute(self):
        ...
        if abs(y_t) > 1:
            print('Nao é possivel atingir a inclinacao a partir da latitude inicial. Calculando a menor possivel')
            y_t = np.sign(y_t)

        self.azimute_final = np.arcsin(y_t)
        self.azimute_inicial = self._estimar_azimute_inicial()

        print(f'Condicao final de azimute de velocidade inercial (grau): {np.rad2deg(self.azimute_final)}')
        print(f'Condicao inicial de azimute de velocidade relativa (grau): {np.rad2deg(self.azimute_inicial)}')
