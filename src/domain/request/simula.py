...
Isp = np.array([251, 271, 315])  # s - Impulso específico dos estagio
# s
...
phi0 = phi0 * np.pi / 180
y_t = np.cos(ingso) / np.cos(delta0)  # Faz um teste de factibilidade usando a inclinacao da orbita e a
# latitude inicial
if abs(y_t) > 1:
...
vigto = np.sqrt(mut * (2 / rpgto - 1 / agto))  # Velocidade de uma orbita de transferencia com 250km de
# altitude
