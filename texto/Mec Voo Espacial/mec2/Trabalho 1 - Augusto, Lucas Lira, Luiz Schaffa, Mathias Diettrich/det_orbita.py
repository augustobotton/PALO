...
def det_orbita(t0,rc0,vc0,mu):
    ...
    if tipo=='e': # Órbita elíptica
        ...
        tau=t0-(E0-e*np.sin(E0))/n
            ...
            tau=t0-(np.tan(theta0/2)**3/6+np.tan(theta0/2)/2)/np.sqrt(mu/p**3) # Eq. 12 da aula 9
    ...
    # Inclinação
    if abs(OMEGA)>1e-3: # Evita divisao por zero ou por um numero proximo de zero
        i=np.arctan2(hc[0]/np.sin(OMEGA),hc[2]) # Eq. 21 da aula 8
    else:
        i=np.arctan2(-hc[1]/np.cos(OMEGA),hc[2]) # Eq. 21 da aula 8
    #
    # Vetor unitário ao longo do vetor excentricidade (no referencial celeste)
    ie=ec/e
    # Vetor unitário ao longo do vetor h (no referencial celeste)
    ih=hc/h
