...
################################################# INPUTS SIMULAÇÃO ################################################################
#   Demora cerca de 5 minutos para dar resultado! Como todas as funções estão no mesmo arquivo, o código não está otimizado       #

Tempo = 150000   #float(input('Informe o tempo da simulacao (s): '))
V0 = 1           #float(input('Informe o valor inicial da velocidade relativa (m/s): '))
phi0 = 80        #float(input('Informe a condicao inicial do angulo de elevacao (graus): '))
...
def dinamica_foguete(t, X):
    ...
    h = r - Requat  # Altitude
    T, _, _, rho, _, Mach, _, _, Kn, _, _,R = atm_padrao(h, V, lc, dT)
...
def propulsao_N_estagios(t, X):
    ...
    if h < 200e3:
        ...
        epsl = -np.arctan2(-np.cos(phi) * np.sin(phii) + np.sin(phi) * np.sin(A) * np.cos(phii) * np.sin(Ai) + np.sin(phi) * np.cos(A) * np.cos(phii) * np.cos(Ai), 
                       np.sin(phi) * np.sin(phii) + np.cos(phi) * np.sin(A) * np.cos(phii) * np.sin(Ai) + np.cos(phi) * np.cos(A) * np.cos(phii) * np.cos(Ai))
    return ft, m, mu, epsl
...
def grav_axisimetrico(r,delta):
    ...
    gr = -(mut) / (r ** 2) - (1 / r) * mut * (-((J2 * Requat ** 2 * (-1 + 3 * np.cos(phi) ** 2)) / (r ** 3)) - (1.5 * J3 * Requat ** 3 * (-3 * np.cos(phi) +    5 * np.cos(phi) ** 3)) / (r ** 4) -(J4 * Requat ** 4 * (3 - 30 * np.cos(phi) ** 2 + 35 * np.cos(phi) ** 4)) / (2 * r ** 5)) + (mut / (r ** 2)) * ((0.5 * J2 * Requat ** 2 * (-1 + 3 * np.cos(phi) ** 2)) / (r ** 2) + (0.5 * J3 * Requat ** 3 * (-3 * np.cos(phi) + 5 * np.cos(phi) ** 3)) / (r ** 3) + (J4 * Requat ** 4 * (3 - 30 * np.cos(phi) ** 2 + 35 * np.cos(phi) ** 4)) / (8 * r ** 4))

    gphi = -(1 / r**2) * mut * (-((3 * J2 * Requat**2 * np.cos(phi) * np.sin(phi))/r**2)+ (0.5 * J3 * Requat**3 * (3 * np.sin(phi) - 15 *np.cos(phi)**2 * np.sin(phi))) /r**3 + (J4 * Requat**4 * (60 * np.cos(phi) * np.sin(phi) - 140 * np.cos(phi)**3 * np.sin(phi))) / (8 * r**4))
...
mp = np.array([77366, 11058, 225.33, 0])  # kg - Massa de propelente dos estagios
Tq1 = 62  # s - NAO MUDA - DADO DOS MOTORES DO 1° ESTAGIO
Tq2 = 64.62  # s - NAO MUDA - DADO DOS MOTORES DO 2° ESTAGIO
Tq3 = 279  # s - TEMPO DE QUEIMA DO 3° ESTAGIO SE ELE IGNITASSE SO UMA VEZ
...
# Simulação
t0 = 0
options = {'rtol': 1e-8, 'atol': 1e-10, 'max_step': 1}
...
def earth_sphere(num_points=100):
    ...
    x = Requat/1000 * np.outer(np.cos(u), np.sin(v))
    y = Requat/1000 * np.outer(np.sin(u), np.sin(v))
    z = Requat/1000 * np.outer(np.ones(np.size(u)), np.cos(v))

    return x, y, z


def plot_trajectory_on_earth(xs, ys, zs):
...
plot_trajectory_on_earth(R0[:,0]/1000,R0[:,1]/1000,R0[:,2]/1000)
