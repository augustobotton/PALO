import parametros
import numpy as np
from matrizTransicaoEstado import matrizTransicaoEstado
from resolveEqBarker import resolveEqBarker
from resolveEqKepler import resolveEqKepler
from resolveEqKeplerHiperbolica import resolveEqKeplerHiperbolica
...
# Vetor de tempo
N=100;t=np.linspace(0,P,N) # Vetor para um período de orbita, 100 divisões
...
for i in range(N):
    ...
    PHI=matrizTransicaoEstado(theta[i])
...
plt.figure(1);
#
plt.subplot(2,2,1);plt.plot(t,E*180/np.pi,label="E")
plt.plot(t,theta*180/np.pi,label="theta")
plt.xlabel('t [s]');plt.ylabel('theta, E [°]')
plt.grid();plt.legend();
#
plt.subplot(2,2,3);plt.plot(t,V[0,:]/1e3,label="v_x")
plt.plot(t,V[1,:]/1e3,label="v_y")
plt.xlabel('t [s]');plt.ylabel('v_x, v_y [km/s]')
plt.grid();plt.legend();
#
plt.subplot(1,2,2);plt.plot(R[0,:]/1e3,R[1,:]/1e3)
plt.plot(xt/1e3,yt/1e3,'b');plt.plot(xt/1e3,-yt/1e3,'b')
...
for i in range(N):
    ...
    # cada tempo
    H[i]=resolveEqKeplerHiperbolica(t[i])
...
plt.figure(2);
#
plt.subplot(2,2,1);plt.plot(t,H*180/np.pi,label="H")
plt.plot(t,theta*180/np.pi,label="theta")
plt.xlabel('t [s]');plt.ylabel('theta, H [°]')
plt.grid();plt.legend();
#
plt.subplot(2,2,3);plt.plot(t,V[0,:]/1e3,label="v_x")
plt.plot(t,V[1,:]/1e3,label="v_y")
...
plt.figure(3);
#
plt.subplot(2,2,1);plt.plot(t,theta*180/np.pi)
